...
/**
 * Builds the fully configured API without binding a port.
 *
 * Extracted so the standalone server (main.ts) and the serverless handler
 * (apps/marketing-web/app/api/v1/[...path]/route.ts) share ONE configuration.
 * Two copies of this would drift, and the first thing to drift silently would
 * be a security header or a rate-limit budget.
 */
export async function createApiApp(): Promise<NestFastifyApplication> {
  ...
  if (!process.env.DATABASE_URL) {
    throw new Error(
      "DATABASE_URL is required. Use a local PostgreSQL or Neon connection string."
    );
  }
  /**
   * The Prisma schema declares `directUrl`, and leaving it unset does NOT fail
   * at startup on its own -- it fails much later, and only on the paths that
   * write outside a tenant context. Self-serve signup is one of them: it was
   * returning a 500 from a row-level-security violation on Property, while
   * every other endpoint behaved perfectly. Refuse to start instead, because a
   * broken signup that only shows up in production is not worth the
   * convenience of an optional variable.
   */
  if (!process.env.DIRECT_URL) {
    throw new Error(
      "DIRECT_URL is required (the Prisma schema declares directUrl). Without it, " +
        "writes that run before a tenant context exists — signup in particular — " +
        "fail against row-level security. Set it to the unpooled connection string."
    );
  }
